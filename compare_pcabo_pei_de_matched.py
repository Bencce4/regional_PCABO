...
except ImportError:  # Older local BoTorch versions do not expose NumericsWarning.
    from botorch.exceptions.warnings import BotorchWarning as NumericsWarning
...
METHOD_BASELINE = "pca_bo_pei_de"
METHOD_CLEAN = "pca_bo_dynamic_orthogonal_clean"
METHODS = (METHOD_BASELINE, METHOD_CLEAN)
RUN_SUFFIX = "pcabo_pei_de_matched"
...
def generate_covar_module(
    ...
    if kernel_ard:
        kernel_kwargs["ard_num_dims"] = active_dim

    if kernel_type == "rbf":
        internal_kernel = RBFKernel(**kernel_kwargs)
    else:
...
def make_pcabo_latent_bounds(
    ...
    x_center = 0.5 * (lb + ub)
    z_center = ((x_center - x_mean) - x_weighted_mean) @ P_r.T
    circumsphere_radius = 0.5 * torch.linalg.norm(torch.clamp(ub - lb, min=1e-12), ord=2)
    return torch.stack([z_center - circumsphere_radius, z_center + circumsphere_radius], dim=0)
...
def sample_initial_design(
    ...
    unit_samples_np = sample_maximin_lhs_unit(dim=dim, n_samples=n0, seed=seed, iterations=olhs_iterations)
    unit_samples = torch.tensor(unit_samples_np, dtype=lb.dtype)
    return lb + (ub - lb) * unit_samples


def sample_lhs_unit(dim: int, n_samples: int, rng: np.random.Generator) -> np.ndarray:
    result = np.empty((n_samples, dim), dtype=float)
    for col in range(dim):
        result[:, col] = (rng.permutation(n_samples) + rng.random(n_samples)) / float(n_samples)
    return result


def sample_maximin_lhs_unit(dim: int, n_samples: int, seed: int, iterations: int) -> np.ndarray:
    rng = np.random.default_rng(seed)
    best_sample = None
    best_min_distance = -np.inf
    best_mean_distance = -np.inf
    for _ in range(max(1, iterations)):
        candidate = sample_lhs_unit(dim=dim, n_samples=n_samples, rng=rng)
        distances = pdist(candidate, metric="euclidean")
        min_distance = float(np.min(distances)) if distances.size else np.inf
        mean_distance = float(np.mean(distances)) if distances.size else np.inf
        if (min_distance > best_min_distance) or (
            math.isclose(min_distance, best_min_distance) and mean_distance > best_mean_distance
        ):
            best_sample = candidate
            best_min_distance = min_distance
            best_mean_distance = mean_distance
    if best_sample is None:
        raise RuntimeError("Failed to generate optimized Latin hypercube initial design.")
    return best_sample
...
def bounds_violation_distance(x: torch.Tensor, bounds: Tuple[torch.Tensor, torch.Tensor]) -> torch.Tensor:
    lb, ub = bounds
    lb = lb.to(dtype=x.dtype, device=x.device).reshape(1, -1)
    ub = ub.to(dtype=x.dtype, device=x.device).reshape(1, -1)
    lower_violation = torch.clamp(lb - x, min=0.0)
    upper_violation = torch.clamp(x - ub, min=0.0)
    return torch.linalg.norm(lower_violation + upper_violation, dim=-1)


class PaperPEI(torch.nn.Module):
    def __init__(
        self,
        gp: SingleTaskGP,
        best_f: torch.Tensor,
        ...
        with warnings.catch_warnings():
            ...
            self.base = ExpectedImprovement(gp, best_f=best_f, maximize=False)
        self.bounds = bounds
        self.mapper = mapper
    ...
    def forward(self, z: torch.Tensor) -> torch.Tensor:
        expected_improvement = self.base(z)
        x = self.mapper(z)
        violation = bounds_violation_distance(x=x, bounds=self.bounds)
        while violation.ndim > expected_improvement.ndim:
            violation = violation.squeeze(-1)
        penalty = self.penalty_weight * violation
        return torch.where(violation <= self.feasibility_tol, expected_improvement, -penalty)
...
def run_pca_bo_pei_de(
    ...
    with EvaluationDatLogger(dat_path=dat_path) as eval_logger:
        ...
        while problem.state.evaluations < budget:
            ...
            x_eval = torch.clamp(mapper_norm(new_z_norm), min=lb, max=ub)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run matched PEI+DE PCA-BO and clean dynamic orthogonal PCA-BO.",
    )
    ...
    parser.add_argument(
        "--kernel-ard",
        action="store_true",
        help="Use ARD lengthscales. Disabled by default to keep the RBF kernel closer to the paper.",
    )
    ...
    parser.add_argument(
        "--olhs-iterations",
        type=int,
        default=OLHS_ITERATIONS,
        help="Number of random LHS designs considered by the maximin OLHS approximation.",
    )
    parser.add_argument("--de-maxiter", type=int, default=DE_MAXITER)
    parser.add_argument("--de-popsize", type=int, default=DE_POPSIZE)
    parser.add_argument("--de-no-polish", action="store_true")
...
def build_method_config(args) -> MethodConfig:
    return MethodConfig(
        ...
        de_polish=not args.de_no_polish,
...
def run_experiment(args) -> Path:
    ...
    config = {
        ...
        "method_descriptions": {
            METHOD_BASELINE: (
                "Paper-style PCA-BO baseline: weighted PCA, latent GP, PEI acquisition, "
                "and scipy differential_evolution acquisition optimization."
            ),
            METHOD_CLEAN: (
                "Same PEI+DE PCA-BO core plus dynamic orthogonal exploration."
            ),
        },
        "kernel": {
            "type": method_config.kernel_type,
            "squared_exponential_match": method_config.kernel_type == "rbf",
            "ard": method_config.kernel_ard,
            "nu": method_config.kernel_nu,
            "lengthscale_bounds": method_config.lengthscale_bounds,
        },
        "pca_backend": "torch.pca_lowrank",
        "initial_design": {
            "sampler": "internal Latin hypercube",
            "optimization": "maximin",
            "iterations": args.olhs_iterations,
            "note": "Dependency-free PyDOE-style OLHS approximation: choose the Latin hypercube with largest minimum pairwise distance.",
        },
        "candidate_proposal": {
            "device": method_config.device,
            "acquisition": "Penalized Expected Improvement (PEI): EI for feasible mapped points, negative distance penalty for infeasible mapped points",
            "optimizer": "scipy.optimize.differential_evolution",
            "de_strategy": method_config.de_strategy,
            "de_maxiter": method_config.de_maxiter,
            "de_popsize": method_config.de_popsize,
            "de_polish": method_config.de_polish,
            "global_bounds": "PCA-BO latent cube centered at the transformed BBOB-box center with circumsphere radius",
        },
        ...
        "outputs": {
            ...
            "orthogonal_stats": f"{METHOD_CLEAN} only",
            "timing": "timing.csv",
        },
    }
    ...
    for fid in args.function_ids:
        ...
        for run_idx in range(args.num_runs):
            ...
            initial_x = sample_initial_design(
                lb=lb,
                ub=ub,
                dim=args.dim,
                n0=args.n0,
                seed=seed,
                olhs_iterations=args.olhs_iterations,
            )
            ...
            if METHOD_CLEAN in selected_methods:
                start = time.perf_counter()
    ...
    print(f"Saved matched PEI+DE PCA-BO outputs under: {run_root}")
    return run_root
