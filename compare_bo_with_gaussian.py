...
METHOD_PCA = "pca_bo_gaussian"
METHOD_BASELINE = "botorch_baseline"
RUN_SUFFIX = "pca_kernel_pcalowrank_orthgauss"
KERNEL_NU = 2.5
KERNEL_LENGTHSCALE_BOUNDS = (0.005, 4.0)
METHOD_LABELS = {
    METHOD_PCA: "PCA-BO + Orthogonal Gaussian",
...
# Fixed orthogonal Gaussian-noise schedule.
...
def apply_orthogonal_gaussian_noise(
    ...
    rank = P_r.shape[0]
    if rank >= dim:
        return torch.clamp(x_manifold, min=lb, max=ub)

    V_r = P_r.T
    identity = torch.eye(dim, dtype=x_manifold.dtype, device=x_manifold.device)
    projector_perp = identity - V_r @ V_r.T

    progress = min(max(evaluations / max(budget, 1), 0.0), 1.0)
    sigma0 = NOISE_SIGMA0_SCALE * torch.mean(ub - lb)
    sigma_t = sigma0 * ((1.0 - progress) ** NOISE_DECAY_ALPHA)
    sigma_value = float(sigma_t.detach().cpu().item())
    if sigma_value <= 0.0:
        return torch.clamp(x_manifold, min=lb, max=ub)

    gaussian = torch.randn(dim, dtype=x_manifold.dtype, device=x_manifold.device) * sigma_t
    noise_perp = projector_perp @ gaussian
    if float(torch.linalg.norm(noise_perp).detach().cpu().item()) <= 1e-12:
        return torch.clamp(x_manifold, min=lb, max=ub)

    x_eval = x_manifold + noise_perp.view(1, -1)
    return torch.clamp(x_eval, min=lb, max=ub)
...
def run_pca_bo(
    ...
    with EvaluationDatLogger(dat_path=dat_path) as eval_logger:
        ...
        while problem.state.evaluations < budget:
            ...
            new_x = mapper(new_z)
            x_eval = apply_orthogonal_gaussian_noise(
                x_manifold=new_x,
                P_r=P_r,
                lb=lb,
                ub=ub,
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run PCA-BO with orthogonal Gaussian noise and official BoTorch baseline with IOH logging.",
    )
...
def run_experiment(args) -> Path:
    ...
    config = {
        ...
        "noise_sigma0_scale": NOISE_SIGMA0_SCALE,
        "noise_decay_alpha": NOISE_DECAY_ALPHA,
